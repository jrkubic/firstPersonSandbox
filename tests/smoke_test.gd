extends SceneTree
## Headless smoke test for the cook loop.
##
...
## frames (await physics_frame) rather than wall time, so the number of
...
func _check_boot() -> bool:
	...
	var overlay: CanvasLayer = get_first_node_in_group("debug_overlay") as CanvasLayer
	if overlay:
	...
	var eggs: int = get_nodes_in_group("food").size()
	var plates: int = get_nodes_in_group("plate").size()
	var pans: int = get_nodes_in_group("pan").size()
	var stoves: int = get_nodes_in_group("stove").size()
...
func _check_deliver(burned_egg: FoodItem, pan: Pan, plate: Plate) -> void:
	...
	var spawned_ok: bool = _check("deliver.fresh_egg",
		egg != null and get_nodes_in_group("food").size() == 1,
		"food count=%d" % get_nodes_in_group("food").size())
	if not spawned_ok:
		return
	await _step(2)
	...
	var new_plate: Plate = get_first_node_in_group("plate") as Plate
	var new_egg: FoodItem = _first_food()
	_check("deliver.respawn",
		new_plate != null and new_egg != null
		and get_nodes_in_group("plate").size() == 1
		and get_nodes_in_group("food").size() == 1
		and new_egg.state == FoodItem.State.RAW,
		"plates=%d eggs=%d" % [get_nodes_in_group("plate").size(), get_nodes_in_group("food").size()])
...
func _check_occupied_slot(pan: Pan) -> void:
	...
	await _step(10)
	_check("occupied.no_extra_egg", get_nodes_in_group("food").size() == 1 and is_instance_valid(slot_egg),
		"%d eggs in the world (slot egg valid=%s)" % [get_nodes_in_group("food").size(), is_instance_valid(slot_egg)])
	_check("occupied.slot_still_full", not _egg_spawner.is_slot_free(), "egg slot reported free")
	_check("occupied.plate_respawned", get_nodes_in_group("plate").size() == 1 and _plate_spawner.is_slot_free() == false,
		"%d plates, plate slot free=%s" % [get_nodes_in_group("plate").size(), _plate_spawner.is_slot_free()])
...
func _xfail(name: String, ok: bool, detail: String = "") -> bool:
	if ok:
		_passed += 1
		print("PASS %s (expected failure now passes)" % name)
	else:
		_xfailed += 1
		print("XFAIL %s: %s" % [name, detail])
	return ok
