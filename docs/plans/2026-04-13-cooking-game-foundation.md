...
- **Pan rim collision.** The pan is a flat cylinder disc with no walls, so food rolls off as soon as the pan is picked up. The CookSlot Area3D logically contains the food for cooking purposes, but physically the food escapes. Add a ring/torus of thin collision walls around the pan's rim, or replace the cylinder collision with a shallow bowl compound shape.
