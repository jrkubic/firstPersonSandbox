...
- **Grab aim forgiveness.** Small items (egg radius 0.07) are fiddly to raycast-click. `grab_range` was bumped from 2.5 to 4.0 during the slice; further forgiveness would require upgrading the ray to a short sphere cast (`PhysicsShapeQueryParameters3D`) for an aim cone rather than a single ray.
...
- **Pan and plate rim collision.** Both the pan and the plate are flat cylinder discs with no walls, so food rolls off as soon as either one is picked up. The CookSlot / FoodContainer Area3Ds logically contain the food for gameplay purposes, but physically the food escapes. Add a ring/torus of thin collision walls around each rim, or replace the cylinder collision with a shallow bowl compound shape.
- **Debug overlay: 1:1 seconds for cook progress.** Current `cook_progress` is normalized 0..1..2 across cook+burn phases. User wants the overlay to show actual elapsed seconds. Tried converting `cook_progress` to seconds directly in 614586a — broke the egg's state transitions and color lerp. Reverted in 3014eff. Alternative: keep `cook_progress` normalized internally but expose a `cook_elapsed_seconds()` helper and have the overlay watch that instead.
- **Centralize magic group name strings.** Several systems look up each other via hard-coded group strings (`grabbable`, `held`, `food`, `pan`, `plate`, `stove`, `debug_overlay`). If a future scene forgets a group, cooking/grabbing/delivery silently fails. Add a `scripts/constants.gd` (or equivalent) exposing `const GROUP_STOVE := &"stove"` etc. and use StringName throughout.
- **Plate-on-pan re-cooks.** Plating a cooked egg and then setting the plate on top of the pan (on the stove) causes the egg to keep cooking toward BURNED because CookSlot sees the egg inside its volume regardless of whether the egg is resting on a plate. Either have CookSlot skip food whose FoodContainer reports it's contained, or use physics layers to ensure food nested in a plate doesn't overlap the cook slot.
...
- **ItemSpawner parents spawned items under StaticBody3D.** `spawn()` uses `get_parent().add_child()`, which puts fresh eggs/plates under Counter/PlateRack in the scene tree. Works in practice but is a scene-tree surprise during debugging. Consider an explicit `@export_node_path` target parent or `get_tree().current_scene.add_child`.
- **Unused `FoodItem.state_changed` signal.** Declared as a public hook for future VFX/audio subscribers but nothing currently connects to it. Either wire a placeholder listener or add a comment noting it's intentional extension surface.
- **Crosshair shape.** Current crosshair is a `+` character rendered via a Label on a CanvasLayer (see `scenes/player.tscn` Crosshair). Replace with a simple 4x4px center dot — a `ColorRect` of size (4, 4) centered via anchors is the cleanest swap.
