...
- **Done 2026-09-11** — `crouch` action (Ctrl / C, held) added to `project.godot`; `Player` eases the `Head` down by `crouch_height_offset` (-0.6 m, rate `crouch_lerp_speed`), swaps the capsule to `crouch_capsule_height` (1.3 m) with its bottom kept on the floor, blocks jumping and scales `move_speed` by `crouch_speed_multiplier` while crouched, and only stands up again once a standing-height `intersect_shape` probe (held bodies ignored) finds headroom; `Player.is_crouched()`, overlay watch `crouched`, smoke checks `crouch.*`. **Crouch movement.** Floor-level items like the egg are awkward to reach because you can't lower the camera. Add `crouch` action + smooth `Head` Y-offset lerp while held. Pairs well with collision shape resizing during crouch.
- **Done 2026-09-11** — when the precise ray misses, `GrabController._find_assist_candidate()` overlaps a `grab_assist_radius` (0.12 m) sphere every `GRAB_ASSIST_STEP` (0.25 m) along the aim line out to `grab_range`, takes the grabbable body closest to the aim line at the first populated sample, and accepts it only if `_has_line_of_sight()` (a World + Items ray) reaches it, so the assist never grabs through walls or counters; `grab_assist_enabled` toggles it; smoke checks `grab.assist_*`. **Grab aim forgiveness.** Small items (egg radius 0.07) are fiddly to raycast-click. `grab_range` was bumped from 2.5 to 4.0 during the slice; further forgiveness would require upgrading the ray to a short sphere cast (`PhysicsShapeQueryParameters3D`) for an aim cone rather than a single ray.
- **Done 2026-09-11** — `[layer_names]` in `project.godot` names World = 1, Player = 2, Items = 3, Detection = 4; static geometry stays on World, the player sits on Player masking World + Items, egg/pan/plate sit on Items masking World + Player + Items, and every detection Area3D sits on no layer with a narrow mask (StoveDetector: World; CookSlot, FoodContainer, DeliveryZone: Items); the grab ray and assist sphere query Items only, the line-of-sight ray World + Items, and the crouch stand-up probe reuses the player mask; group checks stay as the safety net (assignments in the table below). **Dedicated physics layers.** Detection Area3Ds (StoveDetector, CookSlot, FoodContainer, DeliveryZone) currently use default layer 1 / mask 1, so broadphase considers every body in the scene and GDScript filters by group. Fine for the slice, but once there are multiple cookware types and many food items, move each detection role onto its own physics layer and narrow masks accordingly. Keep group checks as a safety net.

  | Layer     | Bit value | Assigned to                                                                                                  | Mask                                                                       |
  |-----------|-----------|--------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------|
  | World     | 1         | Room, Counter, Stove, PlateRack, Pass, OrderBoard (`StaticBody3D`, engine default layer)                     | World (engine default; static bodies do not use it)                        |
  | Player    | 2         | Player (`CharacterBody3D`)                                                                                   | World + Items (5)                                                          |
  | Items     | 4         | Egg, Pan, Plate (`RigidBody3D`)                                                                              | World + Player + Items (7)                                                 |
  | Detection | 8         | Reserved: StoveDetector, CookSlot, FoodContainer and DeliveryZone sit on no layer (`collision_layer = 0`)     | StoveDetector: World (1); CookSlot, FoodContainer, DeliveryZone: Items (4) |

- **Done 2026-09-11** — `pan.tscn` and `plate.tscn` each carry a ring of 12 thin `BoxShape3D` rim segments (pan 0.13 × 0.06 × 0.012 m at r = 0.25, plate 0.115 × 0.025 × 0.01 m at r = 0.22) plus a `TorusMesh` rim so the wall is visible, and the pan handle was finished (mesh lengthened to join the disc, collision cylinder thickened to r = 0.025 and re-centred); a loose egg now rests against the pan rim inside the `CookSlot` (smoke check `cook.egg_stays_in_pan`) and rides in a carried pan or plate until it is tipped out. **Pan and plate rim collision.** Both the pan and the plate are flat cylinder discs with no walls, so food rolls off as soon as either one is picked up. The CookSlot / FoodContainer Area3Ds logically contain the food for gameplay purposes, but physically the food escapes. Add a ring/torus of thin collision walls around each rim, or replace the cylinder collision with a shallow bowl compound shape.
- **Done 2026-09-11** — `FoodItem.cook_elapsed_seconds()` converts the normalized progress back to seconds (`cook_progress * cook_duration` up to COOKED, then `cook_duration + (cook_progress - 1) * burn_duration`) without touching the state machine or the colour lerp; the overlay watches it as `egg.seconds` (`%.1fs`) next to the still-normalized `egg.progress`. **Debug overlay: 1:1 seconds for cook progress.** Current `cook_progress` is normalized 0..1..2 across cook+burn phases. User wants the overlay to show actual elapsed seconds. Tried converting `cook_progress` to seconds directly in 614586a — broke the egg's state transitions and color lerp. Reverted in 3014eff. Alternative: keep `cook_progress` normalized internally but expose a `cook_elapsed_seconds()` helper and have the overlay watch that instead.
- **Done 2026-09-11** — `scripts/groups.gd` (`class_name Groups`, `RefCounted`) exposes `Groups.GRABBABLE` / `FOOD` / `PLATE` / `PAN` / `STOVE` / `HELD` / `DEBUG_OVERLAY` as `StringName` constants and every `add_to_group` / `is_in_group` / `get_first_node_in_group` call in the scripts uses them (named `groups.gd` rather than `constants.gd`); scene files still list the same names as data, e.g. `groups=["stove"]`. **Centralize magic group name strings.** Several systems look up each other via hard-coded group strings (`grabbable`, `held`, `food`, `pan`, `plate`, `stove`, `debug_overlay`). If a future scene forgets a group, cooking/grabbing/delivery silently fails. Add a `scripts/constants.gd` (or equivalent) exposing `const GROUP_STOVE := &"stove"` etc. and use StringName throughout.
- **Done 2026-09-11** — `FoodContainer` increments/decrements `FoodItem.containers` on enter/exit (kept symmetric through `_foods` membership), `FoodItem.is_contained()` reports it, and `CookSlot._physics_process` skips contained food exactly as it skips held food; `CookSlot.has_food()` was added for the test; smoke checks `plate_on_pan.*` and README manual step 10 cover it. **Plate-on-pan re-cooks.** Plating a cooked egg and then setting the plate on top of the pan (on the stove) causes the egg to keep cooking toward BURNED because CookSlot sees the egg inside its volume regardless of whether the egg is resting on a plate. Either have CookSlot skip food whose FoodContainer reports it's contained, or use physics layers to ensure food nested in a plate doesn't overlap the cook slot.
- **Done 2026-09-11** — `ItemSpawner.is_slot_free()` (true when the last spawned item is gone or queued for deletion, is in `held`, or sits more than `slot_radius` = 0.35 m from the spawn point) replaced the global `food` group check; `KitchenLoop._respawn_items()` waits one frame for the delivered items to be freed, then refills each spawner whose slot is free (smoke checks `deliver.respawn`, `deliver.slots_occupied`). **Multi-food respawn race.** `KitchenLoop._on_delivered` checks `food` group emptiness to decide whether to respawn an egg. With a single recipe this works, but with multiple food items in flight the check will skip respawning even when a specific spawner's slot is empty. Replace with per-spawner occupancy checks once recipes diversify.
- **Done 2026-09-11** — `ItemSpawner.spawn_parent_path` (`@export_node_path("Node")`) chooses the parent; both kitchen spawners point it at the `Kitchen` root (`NodePath("../..")`), and when it is empty the spawner falls back to `get_tree().current_scene`, then to its own parent (headless runs); smoke check `deliver.spawn_parent`. **ItemSpawner parents spawned items under StaticBody3D.** `spawn()` uses `get_parent().add_child()`, which puts fresh eggs/plates under Counter/PlateRack in the scene tree. Works in practice but is a scene-tree surprise during debugging. Consider an explicit `@export_node_path` target parent or `get_tree().current_scene.add_child`.
- **Done 2026-09-11** — `FoodItem` connects its own `_on_state_changed` listener, which stamps `last_state_change_msec`; `seconds_since_state_change()` (-1.0 before the first transition) exposes it for overlays and future timing feedback, and the signal remains the public hook for VFX/audio/scoring. **Unused `FoodItem.state_changed` signal.** Declared as a public hook for future VFX/audio subscribers but nothing currently connects to it. Either wire a placeholder listener or add a comment noting it's intentional extension surface.
- **Done 2026-09-11** — the `Label` was replaced by a 4×4 px `ColorRect` named `Dot` (white at 0.85 alpha, `mouse_filter` ignore) centred with the centre anchor preset in `scenes/player.tscn`. **Crosshair shape.** Current crosshair is a `+` character rendered via a Label on a CanvasLayer (see `scenes/player.tscn` Crosshair). Replace with a simple 4x4px center dot — a `ColorRect` of size (4, 4) centered via anchors is the cleanest swap.

- **Grab snap throws pan contents (open, found 2026-09-11).** `GrabController._physics_process` assigns `linear_velocity` / `angular_velocity` directly (caps 20 m/s, 20 rad/s), so on the grab frame a pan with an egg in it hits the egg like a bat and the egg is thrown out before the rim can hold it (`tests/smoke_test.gd` `carry.pan_keeps_egg`, an expected failure). Probed with real physics: `continuous_cd` on the held pan (worse — the swept rim launches the egg ~5 m), velocity ramps of 20-80 m/s^2, and speed caps down to 5 m/s / 4 rad/s; none kept the egg at all of 1.2 / 1.5 / 2.0 m grab distances. Candidate fixes for a hands-on feel pass: apply the pull as a force/impulse limited per frame instead of a velocity write, ramp the orientation control separately from position, or temporarily freeze / reparent CookSlot contents to the pan while it accelerates. Related: even a re-seated egg is launched over the 0.06 m rim by a walk of more than ~0.8 m or a quick 90-degree turn while holding the pan (rim top 0.10 above the pan centre vs. egg centre 0.11); slow turns and short taps keep it. Flip the `_xfail` to `_check` once it holds.

- **Done 2026-09-11 — physics feel fixes from the review probes.** `HoldTarget` moved from `(0.35, -0.35, -0.6)` to `(0.15, -0.3, -1.2)`: with the 0.4 m capsule and 0.75 m-deep counters the old hold point could not reach the pan or the rack plate from any standing spot (15/15 releases landed on the counter), the new one drops in when standing at the stove with a level view (`release.egg_in_pan`). Egg `continuous_cd = true`: a 22 m/s throw (0.37 m per tick vs a 0.2 m wall) tunnelled out of the room 5/12 times, soft-locking the run (`throw.no_tunnel`). `FoodItem.can_sleep = false`: Jolt put an egg resting in a held pan/plate to sleep and left it hanging in mid-air when the container moved away. The design doc's "Large items held at the same offset look huge" note still applies; per-item hold offsets remain open.
